...
def show_file_dialog_notebook() -> Tuple[Optional[str], Optional[str], Optional[str]]:
    """Show a GUI popup to select input Excel file, output notebook file, and buffer source."""
    ...
    # Ask for buffer source
    buffer_choice = messagebox.askyesno(
        "Select Buffer Source",
        (
            "What type of container will you use for stock buffer?\n\n"
            "Yes = 50 mL Tube\n"
            "No = 60 mL Trough"
        ),
    )

    buffer_source = "tube" if buffer_choice else "trough"
    root.destroy()
    return excel_file, output_file, buffer_source
...
def main() -> None:
    ...
    if len(sys.argv) > 1:
        ...
        try:
            if args.notebook:
                generate_liquid_handler_notebook(args.excel_file, args.output, buffer_source=args.buffer_source)
            else:
    ...
    try:
        ...
        if choice:
            ...
            excel_file, output_file, buffer_source = show_file_dialog_notebook()
            if excel_file and output_file and buffer_source:
                generate_liquid_handler_notebook(excel_file, output_file, buffer_source=buffer_source)
                messagebox.showinfo("Success", f"Successfully generated:\n{output_file}")
